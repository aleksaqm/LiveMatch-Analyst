...
3.  **Generisati analitičke uvide:** Umesto prostog opisa, nudiće zaključke poput *"Ovo je serija od 10-0 za domaći tim, izazvana sa tri uzastopne izgubljene lopte gostiju, što je nateralo njihovog trenera da zatraži tajm-aut."*
...
query "findKeyWinningFactors"(String winningTeam, List factors)
    // Uslov 1: Dominacija u skoku
    $stats: TeamStats(teamId == winningTeam, offensiveRebounds > 15)
    factors.add("Dominacija u ofanzivnom skoku (" + $stats.getOffensiveRebounds() + ").");
or
    // Uslov 2: Ključni igrač imao izvanredno veče
    $pStats: PlayerStats(teamId == winningTeam, points > 35)
    factors.add($pStats.getPlayerName() + " je imao fenomenalno poentersko veče (" + $pStats.getPoints() + " poena).");
or
    // Uslov 3: Pobednička serija u poslednjoj četvrtini
    $run: ScoringRun(teamId == winningTeam, quarter == 4, runSize > 10)
    factors.add("Ključna je bila serija od " + $run.getRunSize() + "-0 u poslednjoj četvrtini.");
end
```
Pozivanjem ovog upita, sistem bi sakupio sve zadovoljene uslove i formulisao odgovor, npr. *"Tim A je pobedio zahvaljujući sledećim faktorima: Dominacija u ofanzivnom skoku (16), Ključna je bila serija od 12-0 u poslednjoj četvrtini."*
